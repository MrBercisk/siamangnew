...
class JadwalModel extends Model
{
    ...
    {
        // Lakukan query untuk mendapatkan tanggal bimbingan dari tabel jadwal
        $db = \Config\Database::connect();
        $builder = $db->table('tbl_jadwal');
        $builder->select('tanggal_bimbingan');
        $builder->join('tbl_pendaftaran', 'tbl_pendaftaran.id = tbl_jadwal.pendaftaran_id');
        $builder->where('tbl_pendaftaran.user_id', $user_id);
        $builder->orderBy('tanggal_bimbingan', 'asc'); // Urutkan berdasarkan tanggal_bimbingan secara descending
        $query = $builder->get();

        // Jika query mengembalikan hasil, ambil semua tanggal bimbingan dalam bentuk array
        ...
        // Jika tidak ada tanggal bimbingan yang ditemukan, kembalikan null atau array kosong
        return null;
    }
